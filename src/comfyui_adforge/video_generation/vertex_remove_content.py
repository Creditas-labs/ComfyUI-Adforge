"""
Video Remove Content Node for ComfyUI

Remove content from existing videos using Google GenAI SDK.
"""
...
class VertexVeoRemoveContentNode(IO.ComfyNode):
    """
    Remove content from an existing video.
    """
    ...
    def define_schema(cls) -> IO.Schema:
        ...
        return IO.Schema(
            node_id="VertexVeoRemoveContentNode",
            display_name="Vertex Veo Remove Content",
            category="AdForge/Video Generation",
            description=get_documentation("VertexVeoRemoveContentNode"),
            inputs=[
                ...
                # --- Optional ---
                IO.String.Input("prompt", tooltip=get_tooltip("prompt"), optional=True, force_input=True),
                IO.String.Input(
                    "negative_prompt", tooltip=get_tooltip("negative_prompt"), optional=True, force_input=True
                ),
                IO.String.Input(
                    "output_gcs_uri",
                    default=settings.get_default_gcs_uri("video-remove-content"),
    ...
    def execute(
        ...
        """Remove content from video using mask-based editing."""
        client = utils.get_genai_client()

        if not output_gcs_uri:
            output_gcs_uri = settings.get_default_gcs_uri("video-remove-content")
        ...
        config_params = {
            ...
            "mask": VideoGenerationMask(image=mask, mask_mode=VideoGenerationMaskMode.REMOVE),
        }
        ...
        source_params = {"video": source_video}
        if prompt:
            source_params["prompt"] = prompt

        operation = client.models.generate_videos(
            model=model,
            source=GenerateVideosSource(**source_params),
            config=GenerateVideosConfig(**config_params),
        )

        result = utils.poll_operation(client, operation)
        videos, video_paths, previews = utils.process_genai_results(result, "vertex-remove")
